...
class Controller:
    ...
    def get_max_width_and_half_length (self, Fracture):

        self.fracture = Fracture

        w_max = np.max(self.fracture.wHist) 

        front_cords = self.fracture.Ffront
        max_len_value = 0

        for i in range( front_cords.shape[0] ):
         
            for j in range( front_cords.shape[0] ):
                current_len_value = ((  (front_cords[i][0] - front_cords[j][0])**2 
                                + (front_cords[i][1] - front_cords[j][1])**2 )
                                )**(1/2)
                if (max_len_value < current_len_value):
                     max_len_value = current_len_value


        half_len = max_len_value / 2
            
        return w_max, half_len
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def MakeCSimple (self, r ):
        C = self.C
        mesh = self.fracture.mesh
        hmin = min (mesh.hx, mesh.hy)
        CSimp = np.diag(np.diag(C)) 
        if (r < hmin):
            return CSimp

        for i in range (0, mesh.NumberOfElts):
            fournieb = mesh.NeiElements
            for j in range(0, 4): # check left, right, down, up neib
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[i][j], 0])**2 +\
                    (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[i][j], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[i][j]] = C[i][fournieb[i][j]]

            """ if(i>0):
                print('left ', CSimp [i][fournieb[i][0]],'right ', CSimp [i][fournieb[i][1]], ' down', CSimp [i][fournieb[i][2]] ,
                      'up ', CSimp [i][fournieb[i][3]]) """

            j = fournieb[i][3]      # up neib  
            for n in 0, 1, 3:  # add to up nieb l, r, u nieb 
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[j][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[j][n], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[j][n]] = C[i][fournieb[j][n]]

            upleft = fournieb[j][0]   # up l neib     
            for n in 0, 3:  # add to ul nieb l, u nieb 
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[upleft][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[upleft][n], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[upleft][n]] = C[i][fournieb[upleft][n]]

            upleftleft = fournieb[upleft][0]   # ull neib    
            for n in 2, 3:  # add to upll nieb d, u nieb 
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[upleftleft][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[upleftleft][n], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[upleftleft][n]] = C[i][fournieb[upleftleft][n]]        

            upright = fournieb[j][1]       # ur 
            for n in 1, 3:  # add to upr nieb r, u nieb
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[upright][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[upright][n], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[upright][n]] = C[i][fournieb[upright][n]]     

            uprightright = fournieb[upright][1]    # u rr   
            for n in 2, 3:  # add to uprr nieb d, u nieb 
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[uprightright][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[uprightright][n], 1])**2 )
                if ( distNieb <= r ):
                    CSimp [i][fournieb[uprightright][n]] = C[i][fournieb[uprightright][n]]               

            j = fournieb[i][2]      # down neib  
            for n in range(0,3):  # add to down nieb l, r, d nieb 
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[j][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[j][n], 1])**2 )    
                if ( distNieb <= r ):
                    CSimp [i][fournieb[j][n]] = C[i][fournieb[j][n]]

            downleft = fournieb[j][0]          #down l
            for n in 0, 2:  # add to dl neib l, d neib
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[downleft][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[downleft][n], 1])**2 )    
                if ( distNieb <= r ):
                    CSimp [i][fournieb[downleft][n]] = C[i][fournieb[downleft][n]]

            downleftdown = fournieb[downleft][2]  # dld
            n = 0 # l neib
            distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[downleftdown][n], 0])**2 +\
                    (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[downleftdown][n], 1])**2 )    
            if ( distNieb <= r ):
                CSimp [i][fournieb[downleftdown][n]] = C[i][fournieb[downleftdown][n]]
            
            downright = fournieb[j][1]         #dr
            for n in 1, 2:  # add to dr r, d
                distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[downright][n], 0])**2 +\
                        (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[downright][n], 1])**2 )    
                if ( distNieb <= r ):
                    CSimp [i][fournieb[downright][n]] = C[i][fournieb[downright][n]]

            downrightdown = fournieb[downright][2]  #drd
            n = 1 # add to drd r
            distNieb = math.sqrt( (mesh.CenterCoor[i, 0] - mesh.CenterCoor[fournieb[downrightdown][n], 0])**2 +\
                    (mesh.CenterCoor[i, 1] - mesh.CenterCoor[fournieb[downrightdown][n], 1])**2 )    
            if ( distNieb <= r ):
                CSimp [i][fournieb[downrightdown][n]] = C[i][fournieb[downrightdown][n]]
    ...
    def MakeCSimpleExclude (self, percent):
        C = self.C
        cmax = np.amax(C)
        cmax = abs(cmax)
    ...
    def run(self):
        ...
        mf.T.tic('make elasticity matrix')
        if self.C is None:
        ...
        mf.T.toc('make elasticity matrix')

        """ plt.spy(self.C)
        plt.show()    
        """
        """ plt.imshow(np.log(abs ( self.C)),cmap = mf.cmapp)
        plt.colorbar()
        plt.show()"""

        
        #mf.r = 2*self.fracture.mesh.hx

        """ mf.T.tic('make C simple')    
        mf.Csimple = self.MakeCSimple( mf.r)
        mf.T.tic('make C simple')     """

        """ mf.T.tic('make C simple')    
        mf.Csimple = self.MakeCSimpleExclude( 30 )
        mf.T.tic('make C simple')  """

        #print(self.C)

        if (mf.Ctemplate == 'diagonal'):

            mf.T.tic('make C simple')    

            mf.Csimple = np.diag(np.diag(self.C))   

            mf.T.toc('make C simple')

        if (mf.Ctemplate == '3point'):
            ...
            for i in range(1, len(self.C)-1):
                mf.Csimple[i,i+1] = self.C[i][i+1]
                mf.Csimple[i,i-1] = self.C[i][i-1]

            mf.Csimple[0,1] = self.C[0][1]
            mf.Csimple[len(self.C)-1][len(self.C) -2] = self.C[len(self.C)-1][len(self.C)-2]

            mf.T.toc('make C simple')    

        if (mf.Ctemplate == '5point'):
            ...
            # не учтен случай когда nx != ny
            for i in range( 0, len(self.C)-self.fracture.mesh.nx):
                mf.Csimple[i][ i + self.fracture.mesh.nx] = self.C[i][i + self.fracture.mesh.nx]

                mf.Csimple[self.fracture.mesh.nx + i][i] = self.C[self.fracture.mesh.nx + i][i]

            """ plt.imshow(mf.Csimple)
            plt.colorbar()
            plt.show() """

            mf.T.toc('make C simple')    

        if (mf.Ctemplate == '3x3point'):
            ...
            # не учтен случай когда nx != ny
            for i in range( 0, len(self.C)-self.fracture.mesh.nx - 1):
                mf.Csimple[i][ i + self.fracture.mesh.nx] = self.C[i][i + self.fracture.mesh.nx]  
                mf.Csimple[i][ i + self.fracture.mesh.nx + 1] = self.C[i][i + self.fracture.mesh.nx + 1]  
                mf.Csimple[i][ i + self.fracture.mesh.nx - 1] = self.C[i][i + self.fracture.mesh.nx - 1]  
                #print(i, self.C[i][i + self.fracture.mesh.nx - 1])

                mf.Csimple[self.fracture.mesh.nx + i][i] = self.C[self.fracture.mesh.nx + i][i] 
                mf.Csimple[i + self.fracture.mesh.nx + 1][ i] = self.C[i + self.fracture.mesh.nx + 1][i]  
                mf.Csimple[i + self.fracture.mesh.nx - 1][i] = self.C[i + self.fracture.mesh.nx - 1][i] 
                

            i = len(self.C)-self.fracture.mesh.nx - 1
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            ...
            """ plt.imshow( np.log(abs(mf.Csimple)), cmap = mf.cmapp)
            plt.colorbar()
            plt.show() """
            """ plt.spy(mf.Csimple)
            plt.show() """
        ...
        if (mf.Ctemplate == '9point'):
            ...
            for i in range(2, len(self.C)-2):
                mf.Csimple[i,i+1] = self.C[i][i+1]
                mf.Csimple[i,i+2] = self.C[i][i+2]
                mf.Csimple[i,i-1] = self.C[i][i-1]
                mf.Csimple[i,i-2] = self.C[i][i-2]

            mf.Csimple[0,1] = self.C[0][1]
            mf.Csimple[0,2] = self.C[0][2]

            mf.Csimple[1,0] = self.C[1][0]
            mf.Csimple[1,2] = self.C[1][2]
            mf.Csimple[1,3] = self.C[1][3]

            mf.Csimple[len(self.C)-1][len(self.C) -2] = self.C[len(self.C)-1][len(self.C)-2]
            mf.Csimple[len(self.C)-1][len(self.C) -3] = self.C[len(self.C)-1][len(self.C)-3]

            mf.Csimple[len(self.C)-2][len(self.C) -1] = self.C[len(self.C)-2][len(self.C)-1]
            mf.Csimple[len(self.C)-2][len(self.C) -3] = self.C[len(self.C)-2][len(self.C)-3]
            mf.Csimple[len(self.C)-2][len(self.C) -4] = self.C[len(self.C)-2][len(self.C)-4]
            ...
            for i in range( 0, len(self.C)- 2*self.fracture.mesh.nx):
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx] = self.C[i][i + 2*self.fracture.mesh.nx]

                mf.Csimple[ i + 2*self.fracture.mesh.nx][i] = self.C[ i + 2*self.fracture.mesh.nx][i] 
        ...
        if (mf.Ctemplate == '5x5point'):
            ...
            #     
            # не учтен случай когда nx != ny
            for i in range( 0, len(self.C)-self.fracture.mesh.nx - 2):
                mf.Csimple[i][ i + self.fracture.mesh.nx] = self.C[i][i + self.fracture.mesh.nx]  
                mf.Csimple[i][ i + self.fracture.mesh.nx + 1] = self.C[i][i + self.fracture.mesh.nx + 1]  
                mf.Csimple[i][ i + self.fracture.mesh.nx - 1] = self.C[i][i + self.fracture.mesh.nx - 1]  
                mf.Csimple[i][ i + self.fracture.mesh.nx + 2] = self.C[i][i + self.fracture.mesh.nx + 2]  
                mf.Csimple[i][ i + self.fracture.mesh.nx - 2] = self.C[i][i + self.fracture.mesh.nx - 2]

                mf.Csimple[self.fracture.mesh.nx + i][i] = self.C[self.fracture.mesh.nx + i][i] 
                mf.Csimple[i + self.fracture.mesh.nx + 1][ i] = self.C[i + self.fracture.mesh.nx + 1][i]  
                mf.Csimple[i + self.fracture.mesh.nx - 1][i] = self.C[i + self.fracture.mesh.nx - 1][i] 
                mf.Csimple[i + self.fracture.mesh.nx + 2][ i] = self.C[i + self.fracture.mesh.nx + 2][i]  
                mf.Csimple[i + self.fracture.mesh.nx - 2][i] = self.C[i + self.fracture.mesh.nx - 2][i] 

            i = len(self.C)-self.fracture.mesh.nx - 2
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            mf.Csimple[i][j-1] = self.C[i][j-1]
            mf.Csimple[j-1][i] = self.C[j-1][i]
            mf.Csimple[i][j-2] = self.C[i][j-2]
            mf.Csimple[j-2][i] = self.C[j-2][i]
            mf.Csimple[i][j-3] = self.C[i][j-3]
            mf.Csimple[j-3][i] = self.C[j-3][i]
            ...
            i = len(self.C)-self.fracture.mesh.nx 
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            mf.Csimple[i][j-1] = self.C[i][j-1]
            ...
            for i in range( 0, len(self.C)-2*self.fracture.mesh.nx - 2):
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx] = self.C[i][i + 2*self.fracture.mesh.nx]  
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx + 1] = self.C[i][i + 2*self.fracture.mesh.nx + 1]  
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx - 1] = self.C[i][i + 2*self.fracture.mesh.nx - 1]  
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx + 2] = self.C[i][i + 2*self.fracture.mesh.nx + 2]  
                mf.Csimple[i][ i + 2*self.fracture.mesh.nx - 2] = self.C[i][i + 2*self.fracture.mesh.nx - 2]

                mf.Csimple[2*self.fracture.mesh.nx + i][i] = self.C[2*self.fracture.mesh.nx + i][i] 
                mf.Csimple[i + 2*self.fracture.mesh.nx + 1][ i] = self.C[i + 2*self.fracture.mesh.nx + 1][i]  
                mf.Csimple[i + 2*self.fracture.mesh.nx - 1][i] = self.C[i + 2*self.fracture.mesh.nx - 1][i] 
                mf.Csimple[i + 2*self.fracture.mesh.nx + 2][ i] = self.C[i + 2*self.fracture.mesh.nx + 2][i]  
                mf.Csimple[i + 2*self.fracture.mesh.nx - 2][i] = self.C[i + 2*self.fracture.mesh.nx - 2][i] 

            i = len(self.C)-2*self.fracture.mesh.nx - 2
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            mf.Csimple[i][j-1] = self.C[i][j-1]
            ...
            i = len(self.C)- 2*self.fracture.mesh.nx - 1
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            mf.Csimple[i][j-1] = self.C[i][j-1]
            ...
            i = len(self.C) - 2*self.fracture.mesh.nx 
            j = len(self.C) - 1
            mf.Csimple[i][j] = self.C[i][j]
            mf.Csimple[j][i] = self.C[j][i]
            mf.Csimple[i][j-1] = self.C[i][j-1]
        ...
        if (mf.Ctemplate == 'peirce class II'):
